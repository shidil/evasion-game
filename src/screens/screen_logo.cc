...
void UpdateLogoScreen(void)
{
    ...
    else if (state == 3)            // State 3: Letters appearing (one by one)
    {
        ...
        if (frames_counter/10)       // Every 12 frames, one more letter!
        {
            lettersCount++;
            frames_counter = 0;
        }
...
void DrawLogoScreen(void)
{
    if (state == 0)
    {
        if ((frames_counter/10)%2) DrawRectangle(logoPositionX, logoPositionY, 16, 16, BLACK);
    }
    else if (state == 1)
    {
    ...
    else if (state == 4)
    {
        ...
        if (frames_counter > 20) DrawText("powered by", logoPositionX, logoPositionY - 27, 20, Fade(DARKGRAY, alpha));
    }
}
